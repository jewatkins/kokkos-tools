...
//      : name(std::move(name_in)), ptr(ptr_in), size(size_in), frame(frame_in) {}
  Allocation(std::string&& name_in, const void* ptr_in, std::uint64_t size_in)
      : name(std::move(name_in)), ptr(ptr_in), size(size_in) {}
...
//    auto res = alloc_set.emplace(Allocation(std::move(name), ptr, size, frame));
    auto res = alloc_set.emplace(Allocation(std::move(name), ptr, size));
    assert(res.second);
    total_size += size;
  }
...
//    auto key = Allocation(std::move(name), ptr, size, frame);
    auto key = Allocation(std::move(name), ptr, size);
...
//           << '\n';  // convert bytes to kB
        ss << "MAX MEMORY ALLOCATED: " << double(total_size) / 1024.0 / 1024.0 << " MiB"
...
//  }
  State() {}
  ~State() {
    int space = SPACE_HOST;
    //int space = SPACE_CUDA;
    //int space = SPACE_HIP;
    //int space = SPACE_SYCL;
    //int space = SPACE_OMPT;
